...
def plot_ghg_coefficient_plots(output_dir):
    """
    Plot model coefficients for GHG variables (pCO2, pCH4).
    ...
    models = [
        {
            ...
            'coefs': [0.2, 0.15, 0.1, 0.35],
            'ci_low': [-0.2, -0.25, -0.3, 0.05],
            'ci_high': [0.6, 0.55, 0.5, 0.65]
        },
        {
            'name': 'pCH$_4$',
            'color': 'green',
            'predictors': ['discharge', 'temp', 'rip_open', 'din', 'do.def', 'din:temp'],
            'coefs': [0.15, 0.35, -0.1, 0.2, 0.45, 0.3],
            'ci_low': [-0.25, 0.05, -0.4, -0.1, 0.15, 0.0],
            'ci_high': [0.55, 0.65, 0.2, 0.5, 0.75, 0.6]
        }
    ]
    ...
    # Save figure
    output_path = os.path.join(output_dir, 'figure_05_ghg_coefficients.jpg')
...
def main():
    ...
    # Create plot
    plot_ghg_coefficient_plots(output_dir)
